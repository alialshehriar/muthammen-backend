...
import { generateReferralCode } from '../utils/refCode.js';
export class WaitlistService {
    ...
    static async register(input) {
        ...
        try {
            ...
            const existingUser = await client.query('SELECT id, email, referral_code FROM waitlist_signups WHERE email = $1', [input.email]);
            ...
            while (codeExists && attempts < 10) {
                const result = await client.query('SELECT id FROM waitlist_signups WHERE referral_code = $1', [referralCode]);
            ...
            // Insert new signup
            const insertResult = await client.query(`INSERT INTO waitlist_signups 
         (name, email, phone, city, referral_code, referred_by, referral_count, referral_tier)
         VALUES ($1, $2, $3, $4, $5, $6, 0, 'none')
         RETURNING *`, [input.name, input.email, input.phone || null, input.city || null, referralCode, input.referred_by || null]);
            const newSignup = insertResult.rows[0];
            // If referred by someone, update their referral count
            if (input.referred_by) {
                await client.query(`UPDATE waitlist_signups 
           SET referral_count = referral_count + 1,
               referral_tier = CASE
                 ...
                 ELSE 'none'
               END
           WHERE referral_code = $1`, [input.referred_by]);
            }
            // Log event
            await client.query(`INSERT INTO events (event_type, user_email, metadata)
         VALUES ('waitlist_signup', $1, $2)`, [input.email, JSON.stringify({ referral_code: referralCode, referred_by: input.referred_by })]);
    ...
    static async getStats() {
        ...
        SELECT 
          ...
          COUNT(*) FILTER (WHERE referral_tier = 'bronze') as bronze_users
